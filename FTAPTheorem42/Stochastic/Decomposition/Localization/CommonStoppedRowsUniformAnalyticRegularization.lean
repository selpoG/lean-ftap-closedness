...
/-!
# Generic null-set regularization and Jordan data

This module regularizes the raw residual supplied by the source-independent
analytic interface.  The cumulative variation and Jordan paths are shared by
the bounded and random-envelope routes; source-specific producers remain in
their own modules.  Predictability is intentionally not asserted here.
-/
...
/-! ## The generic regularized raw certificate -/

structure CommonStoppedRowsUniformAnalyticRegularizedRawFiniteVariationData
    {S : Process Omega}
  ...
  raw : CommonStoppedRowsUniformAnalyticRawFiniteVariationData hNested A
  bad_null : mu bad = 0
  ...
  Atilde_eVariation_bound : ∀ omega,
    eVariationOn (Atilde · omega) Set.univ ≤ ENNReal.ofReal (V omega)
  ...
  cumulativeVariation_eq_signedMeasureTotalVariation : ∀ t omega,
    cumulativeVariation t omega =
      (FiniteVariationPath.signedMeasure
        (Atilde_boundedVariation omega)).totalVariation.real (Ioc 0 t)
  ...
  cumulativeVariation_totalVariation_bound : ∀ omega,
    cumulativeVariation T omega ≤ V omega
  ...
  jordanPositive_terminal_bound : ∀ omega,
    0 ≤ Aplus T omega ∧ Aplus T omega ≤ V omega
  jordanNegative_terminal_bound : ∀ omega,
    0 ≤ Aminus T omega ∧ Aminus T omega ≤ V omega
  jordanPositive_terminal_memLp : MemLp (Aplus T) (2 : ENNReal) mu
  jordanNegative_terminal_memLp : MemLp (Aminus T) (2 : ENNReal) mu

/-! ## Generic null-set regularization and Jordan construction -/

theorem commonStoppedRowsUniformAnalytic_regularizeRawFiniteVariation
    {S : Process Omega}
    {F : Filtration NNReal (inferInstance : MeasurableSpace Omega)}
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    [SigmaFiniteFiltration mu F]
    ...
    (hNested : CommonStoppedRowsUniformAnalyticNestedGridVariationData
      data v Z Nbar Bbar Xbar M cutoff)
    (hRaw : CommonStoppedRowsUniformAnalyticRawFiniteVariationData hNested A) :
  ...
  have hAtildeVariationBound : ∀ omega,
      eVariationOn (Atilde · omega) Set.univ ≤ ENNReal.ofReal (V omega) := by
    intro omega
  ...
  have hCumulativeRight : ∀ omega t,
      ContinuousWithinAt (localVariation Atilde · omega)
        (Ici t) t :=
    commonStopCumulativeVariation_rightContinuous hAtildeBV hAtildeRight
  ...
  have hCumulativeConst : ∀ omega t, T ≤ t →
      localVariation Atilde t omega =
        localVariation Atilde T omega :=
    commonStopCumulativeVariation_constant_after hAtildeBV hAtildeConst
  have hCumulativeBound : ∀ omega,
      localVariation Atilde T omega ≤ V omega := by
    intro omega
    by_cases hω : omega ∈ bad
    · have hZero : localVariation Atilde T omega = 0 := by
        ...
        simp
      rw [hZero]
      exact data.residual_nonneg omega
    · have hRegω :
          (A 0 omega = 0) ∧
            (∀ t, T ≤ t → A t omega = A T omega) ∧
            eVariationOn (A · omega) Set.univ ≤ ENNReal.ofReal (V omega) := by
      ...
      have hToReal :
          (eVariationOn (A · omega) Set.univ).toReal ≤ V omega := by
        calc
          (eVariationOn (A · omega) Set.univ).toReal ≤
              (ENNReal.ofReal (V omega)).toReal :=
            ENNReal.toReal_mono ENNReal.ofReal_ne_top hRegω.2.2
          _ = V omega := ENNReal.toReal_ofReal (data.residual_nonneg omega)
  ...
  have hJordanDecomp : ∀ t omega,
    ...
    ring
  have hAplusBound : ∀ omega, 0 ≤ Aplus T omega ∧ Aplus T omega ≤ V omega := by
    intro omega
    ...
    have hsum : Aplus T omega + Aminus T omega =
      ...
      dsimp [Aplus, Aminus, commonStopJordanPositive,
        commonStopJordanNegative]
      ring
    have hnonneg : 0 ≤ Aplus T omega := by
  ...
  have hAminusBound : ∀ omega, 0 ≤ Aminus T omega ∧ Aminus T omega ≤ V omega := by
    intro omega
    ...
    have hsum : Aplus T omega + Aminus T omega =
      ...
      dsimp [Aplus, Aminus, commonStopJordanPositive,
        commonStopJordanNegative]
      ring
    have hnonneg : 0 ≤ Aminus T omega := by
    ...
    have hleSum : Aminus T omega ≤
      ...
      linarith
    exact ⟨hnonneg, hleSum.trans (hCumulativeBound omega)⟩
  have hAplusMem : MemLp (Aplus T) (2 : ENNReal) mu := by
    apply memLp_of_ae_norm_le_nonnegative_envelope
    · exact ((hAplusStrong T).mono (F.le T)).aestronglyMeasurable
    · exact data.residual_memLp
    · exact Filter.Eventually.of_forall data.residual_nonneg
    · filter_upwards [] with omega
      rw [Real.norm_eq_abs, abs_of_nonneg (hAplusBound omega).1]
      exact (hAplusBound omega).2
  have hAminusMem : MemLp (Aminus T) (2 : ENNReal) mu := by
    apply memLp_of_ae_norm_le_nonnegative_envelope
    · exact ((hAminusStrong T).mono (F.le T)).aestronglyMeasurable
    · exact data.residual_memLp
    · exact Filter.Eventually.of_forall data.residual_nonneg
    · filter_upwards [] with omega
      rw [Real.norm_eq_abs, abs_of_nonneg (hAminusBound omega).1]
      exact (hAminusBound omega).2
  ...
  exact
    { raw := hRaw
      ...
      Atilde_eVariation_bound := hAtildeVariationBound
      ...
      cumulativeVariation_eq_signedMeasureTotalVariation := by
        intro t omega
        exact commonStopCumulativeVariation_eq_totalVariation
      ...
      cumulativeVariation_totalVariation_bound := hCumulativeBound
      ...
      jordanPositive_terminal_bound := hAplusBound
      jordanNegative_terminal_bound := hAminusBound
      jordanPositive_terminal_memLp := hAplusMem
      jordanNegative_terminal_memLp := hAminusMem }
